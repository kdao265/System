...
The [V1 operator authorization contract](operator-authorization-v1.md) separates request identity from private database capability grants. Explicit Level Policy assignment requires active `level_policy_assign`; no user receives it automatically. Its isolated executor has enumerated target EXP/history/configuration reads and assignment/milestone/unlock inserts, all bound to the validated target in one transaction. Callers receive only command EXECUTE, with no general cross-owner table access. EXP writes, redemption, definition edits and grant management remain forbidden. The [reward event payload V1 contract](level-reward-event-payload-v1.md) freezes the four existing event shapes and retry rules. The assignment-recognition conflict is resolved; these contracts do not enable runtime behavior.
